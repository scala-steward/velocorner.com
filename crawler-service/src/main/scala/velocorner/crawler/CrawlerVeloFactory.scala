...
class CrawlerVeloFactory[F[_]: Async](client: Client[F]) extends Crawler[F] with Http4sClientDsl[F] {
  ...
  override def products(searchTerm: String, limit: Int): F[List[ProductDetails]] = {
    ...
    val uri =
      s"https://eu1-search.doofinder.com/5/search?hashid=406cfaeeed85ea04f6d00b37c67676e0&query_counter=6&page=1&rpp=30&transformer=&session_id=cf831bcf7ad2d7021bdbb28f61c6fbf0&query=$search"
