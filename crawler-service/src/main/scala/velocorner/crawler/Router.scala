...
class Router[F[_]: {Async, Parallel, Logger}](crawlers: List[Crawler[F]]) {
  ...
  private val searchServerEndpoint: ServerEndpoint[Any, F] = searchEndpoint.serverLogic { term =>
    term.trim match {
  ...
  private val supportedServerEndpoint: ServerEndpoint[Any, F] = supportedEndpoint.serverLogicSuccess { _ =>
    crawlers.map(_.market()).pure[F]
  }

  private val apiRoutes: HttpRoutes[F] = Http4sServerInterpreter[F]().toRoutes(List(searchServerEndpoint, supportedServerEndpoint))
