...
class CrawlerBike24[F[_]: Async](_client: Client[F]) extends Crawler[F] {
  ...
  private val playwrightScript = playwrightDir.resolve("bike24-fetch.mjs")
  ...
  private def fetchWithPlaywright(searchTerm: String): F[String] =
    Async[F].blocking {
      ...
      if (!finished) {
        ...
        throw new IllegalStateException(s"Bike24 Playwright fetch timed out for [$searchTerm]")
      }
      ...
      if (exitCode != 0) {
        ...
        throw new IllegalStateException(s"Bike24 Playwright fetch failed for [$searchTerm]$details")
      }
  ...
  override def products(searchTerm: String, limit: Int): F[List[ProductDetails]] =
    fetchWithPlaywright(searchTerm)
      .map(CrawlerBike24.parseProducts(_, limit))
      .handleErrorWith { err =>
        logger.warn(err)(s"Bike24 Playwright crawler failed for [$searchTerm]").as(Nil)
      }
}
